...
class MacroTools {
	...
	/**
		Ensures that `.gitignore` contains the Spilehx header and a path entry.

		This runs at compile time. The `.gitignore` file is created in the
		compiler's current working directory when missing. Existing content is
		preserved, and the header `# Auto generated by Spilehx lib` and the
		normalized entry are appended only when they are not already present as
		complete trimmed lines.

		The `path` argument is normalized before comparison and insertion, but it
		is not validated with `validateProjectPath`; callers should pass entries
		that are appropriate for a Git ignore file.

		@param path The path pattern or file path to add to `.gitignore`.
		@throws haxe.macro.Error If `.gitignore` cannot be created or if the
			project `.gitignore` path fails validation.
		@throws haxe.Exception If reading or rewriting `.gitignore` fails.
	 */
	public static function ensureGitIgnoreEntry(path:String):Void {
